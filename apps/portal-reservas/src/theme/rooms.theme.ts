...
export const ROOM_LIST_STYLES = {
  section:
    "relative w-full max-w-5xl mx-auto px-6 py-16 mt-[180px] animate-in fade-in slide-in-from-bottom-12 duration-700 fill-mode-both",
  header:
    "flex flex-col md:flex-row md:items-end justify-between mb-12 pb-6 border-b-2 border-emerald-900/10",
  ...
  countBadge:
    "mt-6 md:mt-0 px-4 py-2 bg-neutral-100 rounded-lg text-neutral-600 font-bold flex items-center gap-2",
...
export const ROOM_CARD_STYLES = {
  ...
  urgencyBadge:
    "absolute top-4 left-4 bg-[#7a1313] text-white px-4 py-2 rounded-xl font-bold text-sm shadow-xl flex items-center gap-2 animate-in slide-in-from-top-2",
  urgencyIcon: "w-4 h-4",
  ...
  title:
    "text-3xl font-black text-neutral-900 leading-none group-hover:text-emerald-800 transition-colors",
  metaRow: "flex items-center gap-4 mb-6",
  typeChip:
    "px-3 py-1 bg-neutral-100 text-neutral-700 font-bold text-sm rounded-lg border border-neutral-200",
  ...
  priceTier:
    "mt-auto flex flex-col sm:flex-row items-end sm:items-center justify-between border-t-2 border-neutral-100 pt-8 gap-6",
