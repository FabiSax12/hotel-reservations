...
export function RoomDetailFooter({ room, reserveRooms, price, isPackage }: RoomDetailFooterProps) {
  const { t } = useI18n();
  ...
  return (
    <footer className={ROOM_DETAIL_STYLES.footer}>
      <div className={ROOM_DETAIL_STYLES.priceBlock}>
        <span className={ROOM_DETAIL_STYLES.priceLabel}>
          {isPackage ? t.ROOMS.PACKAGE_TOTAL_LABEL : t.ROOMS.PRICE_LABEL}
        </span>
        {hasDates ? (
          ...
          <span className={ROOM_DETAIL_STYLES.pricePlaceholder}>
            {t.ROOM_DETAIL.PRICE_PLACEHOLDER}
          </span>
        )}
      </div>

      <RoomDetailCta room={room} reserveRooms={reserveRooms} isPackage={isPackage} />
    </footer>
  );
}
