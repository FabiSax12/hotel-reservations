export type RoomsTexts = {
  REALTIME_AVAIL: string;
  OPTIONS_IN: string;
  ALL_DESTINATIONS: string;
  ROOMS_FOUND: string;
  LAST_ROOM: string;
  ONLY_REMAINING: string;
  ROOMS_PLURAL: string;
  TYPE_LABEL: string;
  SQFT_LABEL: string;
  PRICE_LABEL: string;
  CURRENCY: string;
  AVAILABLE_DATES: string;
  SELECT_ACTION: string;
  CAPACITY_LABEL: string;
  AMENITIES_TITLE: string;
  ADMIN_TIP_LABEL: string;
  EXPAND_DETAILS: string;
  COLLAPSE_DETAILS: string;
  CLOSE_DETAILS: string;
  RESERVE_ACTION: string;
  LOADING_RESERVE: string;
  CHECK_DATES_ACTION: string;
  SEE_FREE_DATES: string;
  UNAVAILABLE_LABEL: string;
  QUICK_SEARCH_TITLE: string;
  AVAIL_CALENDAR_TITLE: string;
  GUESTS_LABEL: string;
  CONFIRM_SEARCH: string;
  NO_LOCATION_HINT: string;
  BROWSE_ROOMS: string;
  SEARCHING_ROOMS: string;
  AVAIL_DIALOG_EMPTY: string;
  AVAIL_DIALOG_CLOSE: string;
  VERIFYING: string;
  AVAILABILITY_ERROR: string;
  ADULTS_LABEL: string;
  ADULTS_AGE: string;
  CHILDREN_LABEL: string;
  CHILDREN_AGE: string;
  CANCEL: string;
  DECREASE_ADULTS: string;
  INCREASE_ADULTS: string;
  DECREASE_CHILDREN: string;
  INCREASE_CHILDREN: string;
  GALLERY_IMAGES_LABEL: string;
  PACKAGE_INDICATOR_SAME: string;
  PACKAGE_INDICATOR_MIXED_ONE: string;
  PACKAGE_INDICATOR_MIXED_OTHER: string;
  PACKAGE_TOTAL_PRICE_LABEL: string;
  PACKAGE_ROOMS_LABEL: string;
  ROOMS_OPTIONS_FOUND: string;
};
