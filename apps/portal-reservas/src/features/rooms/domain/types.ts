...
// ─── Room Detail Panel Types (US-DM-05) ─────────────────────────────────────
...
export interface RoomDetailHeaderProps {
  /** Gold uppercase eyebrow (room location, or the package eyebrow). */
  eyebrow: string;
  /** Main serif title. */
  title: string;
  /** Optional secondary line under the title (e.g. package room count). */
  subtitle?: string;
  onClose: () => void;
}
