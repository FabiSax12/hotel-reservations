...
export function resolveBothDatesProximity(
  ...
  if (distToIn <= distToOut) {
    ...
    if (clickedVal > outVal)
      return { checkIn: checkOut, checkOut: dayStr, nextActive: SEARCH_SECTIONS.CHECK_OUT };
  ...
  if (clickedVal < inVal)
    return { checkIn: dayStr, checkOut: checkIn, nextActive: SEARCH_SECTIONS.CHECK_IN };
