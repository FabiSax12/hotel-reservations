...
export const AMENITIES_FORM_STYLES = {
  ...
  searchInput:
    "w-full pl-11 pr-4 h-14 rounded-2xl border-2 border-white/60 bg-white/40 focus:bg-white/80 focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 transition-all outline-none text-slate-800 placeholder-slate-500 font-medium",
  ...
  customTriggerCard:
    "relative flex flex-col items-center justify-center gap-2 p-5 rounded-2xl border-2 border-dashed border-emerald-900/15 bg-white/40 hover:bg-white/70 hover:border-emerald-500/30 transition-all duration-300 cursor-pointer group min-h-[120px]",
  customTriggerIconWrapper:
    "p-3 rounded-xl bg-emerald-500/5 text-emerald-700/60 group-hover:bg-emerald-500/10 group-hover:text-emerald-700 transition-colors duration-300",
  customTriggerText:
    "font-semibold text-center text-xs text-emerald-950/60 group-hover:text-emerald-950/90 transition-colors duration-300",
