...
export const mockAmenitiesService = {
  ...
  getRoomAmenities: async (roomId: string): Promise<string[]> => {
    ...
    return allRoomAmenities.filter((ra) => ra.room_id === roomId).map((ra) => ra.amenity_id);
  },
