...
export function handlePickDate(
  ...
  if (checkIn && checkOut) {
    if (clicked < inVal) onChange(dayStr, checkOut);
      ...
      if (dIn <= dOut) onChange(dayStr, checkOut);
      else onChange(checkIn, dayStr);
    }
    return field;
  }

  // Direction validation — trigger shake animation if out of order
  if (field === "checkIn" && checkOut && clicked > outVal) {
    triggerInvalid(dayStr);
    return field;
  }
  if (field === "checkOut" && checkIn && clicked < inVal) {
    triggerInvalid(dayStr);
    return field;
  }
