...
export const CALENDAR_STYLES = {
  wrapper: (isHero: boolean) =>
    isHero
      ...
      : "w-[650px] absolute top-[100%] mt-4 left-1/2 -translate-x-1/2 bg-white rounded-[2.1rem] shadow-[0_24px_60px_rgba(4,120,87,0.16)] border border-emerald-100/80 z-50 animate-in fade-in slide-in-from-top-4 duration-300",
  inlineWrapper: (isHero: boolean) =>
    isHero
      ? "w-full max-w-[1150px] bg-white rounded-[2.25rem] border border-emerald-100/70"
      : "w-full min-w-[640px] bg-white rounded-[2rem] border border-emerald-100/80 shadow-[0_18px_45px_rgba(4,120,87,0.14)]",
  ...
  navBtn:
    "p-1.5 rounded-full hover:bg-emerald-50 transition-colors disabled:opacity-30 disabled:cursor-not-allowed text-neutral-600",
  /** Invisible spacer that occupies the same space as a navBtn on the opposite side */
  navSpacer: "w-8",
  monthTitle: (isHero: boolean) => `${isHero ? "text-2xl" : "text-xl"} font-bold text-neutral-900`,

  daysHeader: (isHero: boolean) =>
    `grid grid-cols-7 text-center font-bold ${isHero ? "gap-y-4 gap-x-2 mb-4 text-sm uppercase tracking-widest text-neutral-500" : "gap-y-2 gap-x-1 mb-2 text-[11px] uppercase tracking-widest text-neutral-500"}`,
  ...
  rangeHighlightStart:
    "absolute top-1/2 -translate-y-1/2 right-0 w-1/2 h-[80%] bg-emerald-100 z-0 pointer-events-none",
  rangeHighlightEnd:
    "absolute top-1/2 -translate-y-1/2 left-0 w-1/2 h-[80%] bg-emerald-100 z-0 pointer-events-none",
  rangeHighlightMid:
    "absolute top-1/2 -translate-y-1/2 left-0 right-0 w-full h-[80%] bg-emerald-100 z-0 pointer-events-none",
  hoverRing:
    "absolute h-[85%] aspect-square rounded-full border-2 border-transparent group-hover:border-neutral-800 transition-colors pointer-events-none",
  selectedStart:
    "absolute h-[85%] aspect-square bg-emerald-700 rounded-full z-10 shadow-sm transition-transform active:scale-95 group-hover:scale-105",
  selectedEnd:
    "absolute h-[85%] aspect-square bg-emerald-700 rounded-full z-10 shadow-sm transition-transform active:scale-95 ring-2 ring-emerald-700 ring-offset-1 group-hover:scale-105",
  ...
  tooltipArrow:
    "absolute -bottom-1 left-1/2 -translate-x-1/2 w-2.5 h-2.5 bg-emerald-950 rotate-45 rounded-sm",
